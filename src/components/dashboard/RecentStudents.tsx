...
export default function RecentStudents() {
  ...
  useEffect(() => {
    const fetchRecentStudents = async () => {
      try {
        ...
        const recentStudents = await loadRecentStudentsFromEndpoint();
        if (recentStudents.length > 0) {
          setStudents(recentStudents);
          return;
        }

        const fallbackStudents = await loadRecentStudentsFromList();
        setStudents(fallbackStudents.slice(0, 5));
      } catch (error) {
  ...
  async function loadRecentStudentsFromEndpoint(): Promise<Student[]> {
    try {
      ...
      if (Array.isArray(payload)) {
        return payload
          ...
          .sort((a: any, b: any) => {
            const dateA = new Date(a.created_at || a.user?.created_at || 0).getTime();
            const dateB = new Date(b.created_at || b.user?.created_at || 0).getTime();
            return dateB - dateA;
          })
          .map(mapStudentRecord);
      }
    } catch (error) {
      console.warn("Endpoint /students/recent indisponível:", error);
    }

    return [];
  }

  async function loadRecentStudentsFromList(): Promise<Student[]> {
    try {
      ...
      console.warn("Fallback /students falhou:", error);
    }

    return [];
  }

  function mapStudentRecord(student: any): Student {
    return {
      id: student.id,
      name: student.name,
      email: student.user?.email || student.email,
      role: student.user?.role || student.role || "aluno",
      created_at: student.created_at || student.user?.created_at,
      createdAt: student.created_at || student.user?.created_at,
      registration: student.registration,
      class: student.class?.name || student.class_?.name,
      school: student.school,
      class_: student.class || student.class_,
    };
  }
