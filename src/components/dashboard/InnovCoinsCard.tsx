    ...
    <Card className="hover:shadow-lg transition-shadow duration-300">
      ...
      <CardContent className="pt-0">
        ...
        <div className="space-y-2 mb-4">
          <div className="text-xs sm:text-sm font-medium text-foreground mb-2">Como ganhar mais:</div>
          <div className="flex items-center justify-between p-2 bg-green-50 dark:bg-green-950/30 rounded border border-green-200 dark:border-green-800">
            <div className="flex items-center gap-2 min-w-0">
              <div className="w-5 h-5 sm:w-6 sm:h-6 bg-green-500 rounded-full flex items-center justify-center flex-shrink-0">
                <BookOpen className="w-3 h-3 text-white" />
              </div>
              <span className="text-xs truncate text-foreground">Fazer avaliação</span>
            </div>
            <span className="text-xs font-bold text-green-600 dark:text-green-400 flex-shrink-0">+2 coins</span>
          </div>
          <div className="flex items-center justify-between p-2 bg-blue-50 dark:bg-blue-950/30 rounded border border-blue-200 dark:border-blue-800">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-xs truncate text-foreground">Nota acima de 8</span>
            </div>
            <span className="text-xs font-bold text-blue-600 dark:text-blue-400 flex-shrink-0">+5 coins</span>
          </div>
          <div className="flex items-center justify-between p-2 bg-purple-50 dark:bg-purple-950/30 rounded border border-purple-200 dark:border-purple-800">
            <div className="flex items-center gap-2 min-w-0">
        ...
        {/* Loja Preview */}
        <div className="p-2 sm:p-3 bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-950/30 dark:to-purple-950/30 rounded-lg border border-indigo-200 dark:border-indigo-800">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-4 h-4 bg-indigo-500 rounded-full flex items-center justify-center flex-shrink-0">
              <span className="text-white text-xs">🛍️</span>
            </div>
            <span className="text-xs sm:text-sm font-medium text-indigo-700 dark:text-indigo-400 truncate">Loja de Recompensas</span>
          </div>
          <div className="space-y-1">
            {moedas.loja.map((item, index) => (
              <div key={index} className="flex items-center justify-between">
                <span className={`text-xs ${item.disponivel ? 'text-indigo-600 dark:text-indigo-400' : 'text-muted-foreground'} truncate`}>
                  {item.item}
                </span>
                <span className={`text-xs font-bold ${item.disponivel ? 'text-indigo-700 dark:text-indigo-400' : 'text-muted-foreground'} flex-shrink-0`}>
                  {item.preco} coins
                </span>
              </div>
            ))}
          ...
          <div className="space-y-1 text-xs">
            {moedas.historico.map((transacao, index) => (
              <div key={index} className="flex items-center justify-between py-1">
                <span className="text-muted-foreground truncate">{transacao.acao}</span>
                <span className="text-green-600 dark:text-green-400 font-medium flex-shrink-0">+{transacao.valor}</span>
              </div>
            ))}
          </div>
        </div>
