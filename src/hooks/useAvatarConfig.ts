...
import { useAuth, AvatarConfig } from '@/context/authContext';
...
export const useAvatarConfig = () => {
    const { user, updateAvatarConfig } = useAuth();
    const [config, setConfig] = useState<AvatarConfig>(DEFAULT_CONFIG);
    const [isSaving, setIsSaving] = useState(false);

    useEffect(() => {
        if (user?.avatar_config) {
            setConfig(user.avatar_config);
        } else {
            // Usar nome do usuário como seed padrão se não houver configuração
            setConfig({
                ...DEFAULT_CONFIG,
                seed: user?.name || `user_${Date.now()}`,
            });
        }
    }, [user?.avatar_config, user?.name]);
    ...
    const updateConfig = useCallback((updates: Partial<AvatarConfig>) => {
        setConfig((prev) => {
            ...
            // Garantir que sempre tenha um seed
            if (!newConfig.seed || newConfig.seed.trim() === '') {
                newConfig.seed = user?.name || `user_${Date.now()}`;
            }
            return newConfig;
        });
    }, [user?.name]);
    ...
    }, [config, updateAvatarConfig]);

    const generateAvatarUrl = useCallback((customConfig?: AvatarConfig): string => {
